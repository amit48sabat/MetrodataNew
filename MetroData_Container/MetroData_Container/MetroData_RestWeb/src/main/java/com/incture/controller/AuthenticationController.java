...
public class AuthenticationController {
	...
	public @ResponseBody ResponseDto login(HttpServletRequest request) {

		ResponseDto responseDto = new ResponseDto();
		if (request.getUserPrincipal() != null) {
			String userId = request.getUserPrincipal().getName();
			
			String userData = restInvoker.getDataFromServer("/Users/" + userId);
			 ...
			 */
			JSONObject jsonResult = new JSONObject(userData);
			JSONObject userDetails = new JSONObject(jsonResult.get("name").toString());
			String name = userDetails.get("givenName") + " " + userDetails.get("familyName");

			String emailDetailsJson = jsonResult.get("emails").toString();
			Gson googleJson = new Gson();
			ArrayList javaArrayListFromGSON = googleJson.fromJson(emailDetailsJson, ArrayList.class);
			String email ="";
			Map m;
			for (Object o : javaArrayListFromGSON) {
				m = googleJson.fromJson(o.toString(), Map.class);
				if ((boolean) m.get("primary")) {
					email= (String) m.get("value");
				}
			}
